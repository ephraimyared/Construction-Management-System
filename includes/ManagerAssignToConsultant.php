...
// Reset the consultants result pointer
$stmt_consultants = $connection->prepare("SELECT * FROM users WHERE Role = 'Consultant' AND managed_by_contractor_id = ?");
$stmt_consultants->bind_param("i", $_SESSION['user_id']);
$stmt_consultants->execute();
$consultants_result = $stmt_consultants->get_result();

// Fetch the number of projects assigned to each consultant
$assignment_counts = [];
$result = $connection->query("SELECT contractor_id, COUNT(*) as assignment_count FROM project_assignments WHERE role_in_project = 'Assigned Consultant' GROUP BY contractor_id");
while ($row = $result->fetch_assoc()) {
    $assignment_counts[$row['contractor_id']] = $row['assignment_count'];
}

// Fetch the actual projects assigned to each consultant
$consultant_projects = [];
$projects_query = $connection->query("
    ...
    WHERE pa.role_in_project = 'Assigned Consultant'
    ORDER BY pa.contractor_id, p.project_name
");

while ($row = $projects_query->fetch_assoc()) {
    if (!isset($consultant_projects[$row['contractor_id']])) {
        $consultant_projects[$row['contractor_id']] = [];
    }
    $consultant_projects[$row['contractor_id']][] = [
...
// Create a lookup array for projects that already have consultants assigned
$project_assignments = [];
$assigned_projects_query = $connection->query("
    SELECT project_id, contractor_id, assignment_id 
    FROM project_assignments 
    WHERE role_in_project = 'Assigned Consultant'
");
...
if (isset($_POST['remove_assignment']) && isset($_POST['assignment_id'])) {
    ...
    if ($stmt->execute()) {
        ...
        header("Location: ManagerAssignToConsultant.php?success=removed");
        exit();
    } else {
...
if (isset($_POST['reassign']) && isset($_POST['assignment_id'])) {
    $assignment_id = $_POST['assignment_id'];
    $new_consultant_id = $_POST['new_consultant_id'];
    
    // Update the assignment with the new consultant
    $update_query = "UPDATE project_assignments SET contractor_id = ? WHERE assignment_id = ?";
    $stmt = $connection->prepare($update_query);
    $stmt->bind_param("ii", $new_consultant_id, $assignment_id);
    
    if ($stmt->execute()) {
        $message = "✅ Project successfully reassigned to new consultant!";
        $message_type = "success";
        
        // Refresh the page to update the data
        header("Location: ManagerAssignToConsultant.php?success=reassigned");
        exit();
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_POST['reassign']) && !isset($_POST['remove_assignment'])) {
    ...
    $status = 'Assigned';
    $role_in_project = 'Assigned Consultant';
    $user_id = $_SESSION['user_id']; // Manager ID
    
    // Check if this project is already assigned to this consultant
    $already_assigned = false;
    if (isset($project_assignments[$project_id])) {
        foreach ($project_assignments[$project_id] as $assignment) {
            if ($assignment['contractor_id'] == $consultant_id) {
    ...
    if ($already_assigned) {
        $message = "⚠️ This project is already assigned to this consultant.";
        $message_type = "danger";
    } else {
        // Check if this project already has a consultant assigned
        $has_consultant = isset($project_assignments[$project_id]) && !empty($project_assignments[$project_id]);
        ...
        if (empty($message) || $message_type == "success") {
            if (strtotime($start_date) > strtotime($end_date)) {
                ...
                // If the project already has a consultant, show a warning but still allow the assignment
                if ($has_consultant) {
                    $warning_message = "⚠️ Note: This project already has a consultant assigned. You are adding an additional consultant to work on this project.";
                }
                ...
                $stmt->bind_param("iiissssss", $project_id, $user_id, $consultant_id, $description, $start_date, $end_date, $status, $role_in_project, $file_path);
                if ($stmt->execute()) {
                    ...
                    // Refresh the page to update the data
                    header("Location: ManagerAssignToConsultant.php?success=assigned");
                    exit();
                } else {
...
<html>
<head>
    <title>Assign Task to Consultant</title>
...
<div class="main-container">
    <div class="page-header">
        ...
        <h1><i class="fas fa-user-tie me-2"></i> Assign Tasks to Consultants</h1>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            <i class="fas fa-users"></i> Available Consultants
        </div>
        <div class="card-body p-0">
            <div class="table-container">
                <table class="table mb-0">
                    ...
                    <tbody>
                        <?php 
                        if ($consultants_result->num_rows > 0): 
                        ?>
                            <?php while ($consultant = $consultants_result->fetch_assoc()): ?>
                                <?php
                                // Get the number of projects assigned to the consultant
                                $assignment_count = isset($assignment_counts[$consultant['UserID']]) ? $assignment_counts[$consultant['UserID']] : 0;
                                $has_projects = isset($consultant_projects[$consultant['UserID']]) && !empty($consultant_projects[$consultant['UserID']]);
                                ?>
                                <tr>
                                    <td><?= htmlspecialchars($consultant['UserID']) ?></td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="avatar-circle">
                                                <?= strtoupper(substr($consultant['FirstName'], 0, 1)) ?>
                                            </div>
                                            <?= htmlspecialchars($consultant['FirstName'] . ' ' . $consultant['LastName']) ?>
                                        </div>
                                    </td>
                                        ...
                                        <?php if ($assignment_count > 0): ?>
                                            ...
                                            <?php if ($has_projects): ?>
                                                <div class="project-list">
                                                    <?php foreach(array_slice($consultant_projects[$consultant['UserID']], 0, 2) as $project): ?>
                                                        <span class="project-badge">
                                                            ...
                                                            <div class="action-buttons ms-2">
                                                                <button type="button" class="btn btn-sm reassign-btn" 
                                                                        ...
                                                                        data-consultant-name="<?= htmlspecialchars($consultant['FirstName'] . ' ' . $consultant['LastName']) ?>">
                                                    ...
                                                    <?php if (count($consultant_projects[$consultant['UserID']]) > 2): ?>
                                                        <button type="button" class="btn btn-sm view-all-btn" 
                                                                ...
                                                                data-bs-content="<?php 
                                                                    $content = '';
                                                                    foreach($consultant_projects[$consultant['UserID']] as $project) {
                                                                    ...
                                                                    echo $content;
                                                                ?>">
                                                            View all <?= count($consultant_projects[$consultant['UserID']]) ?>
                                                        </button>
                                                    <?php endif; ?>
                                                </div>
                                            <?php endif; ?>
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="5" class="text-center py-4">No consultants found. Please ask the admin to create consultants first.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
    ...
    <div class="card">
        <div class="card-header">
            <i class="fas fa-tasks"></i> Assign Task to Consultant
        </div>
        <div class="card-body">
            <form method="POST" action="" id="assignmentForm" enctype="multipart/form-data">
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select name="project_id" id="project_id" class="form-select" required>
                            ...
                            if ($projects_result->num_rows > 0) {
                                while ($project = $projects_result->fetch_assoc()): 
                                    // Check if this project already has a consultant assigned
                                    $has_consultant = isset($project_assignments[$project['project_id']]) && !empty($project_assignments[$project['project_id']]);
                                ?>
                                    <option value="<?php echo $project['project_id']; ?>" 
                                            data-start="<?php echo $project['start_date']; ?>" 
                                            data-end="<?php echo $project['end_date']; ?>">
                                        <?php echo htmlspecialchars($project['project_name']); ?>
                                        <?php if ($has_consultant): ?> 
                                            (Has Consultant)
                                        <?php endif; ?>
                                    </option>
                                <?php endwhile;
                        ...
                        <small class="text-muted">Only approved projects are shown. Projects marked with "(Has Consultant)" already have at least one consultant assigned.</small>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="consultant_id" id="consultant_id" class="form-select" required>
                            ...
                            while ($consultant = $consultants_result->fetch_assoc()): 
                                // Get the number of projects assigned to this consultant
                                $count = isset($assignment_counts[$consultant['UserID']]) ? $assignment_counts[$consultant['UserID']] : 0;
                            ?>
                                <option value="<?php echo $consultant['UserID']; ?>">
                        ...
                        <div id="duplicateWarning" class="text-danger mt-2" style="display: none;">
                            <i class="fas fa-exclamation-circle"></i> This consultant is already assigned to this project.
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST" action="" id="reassignForm">
                    ...
                    <div class="mb-3">
                        <p>You are reassigning the project <strong id="modal_project_name"></strong> from <strong id="modal_current_consultant"></strong> to a new consultant.</p>
                    </div>
                    
                    <div class="mb-3">
                        <label for="new_consultant_id" class="form-label">Select New Consultant</label>
                        <select name="new_consultant_id" id="new_consultant_id" class="form-select" required>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST" action="" id="removeForm">
                    ...
                    <p>Are you sure you want to remove the project <strong id="remove_project_name"></strong> from <strong id="remove_consultant_name"></strong>?</p>
...
<script>
    ...
    function openReassignModal(assignmentId, projectName, consultantId, consultantName) {
        ...
        document.getElementById('modal_current_consultant').textContent = consultantName;
        
        // Set the current consultant as selected in the dropdown
        const newConsultantSelect = document.getElementById('new_consultant_id');
        for (let i = 0; i < newConsultantSelect.options.length; i++) {
            if (newConsultantSelect.options[i].value == consultantId) {
                newConsultantSelect.options[i].disabled = true;
            } else {
                newConsultantSelect.options[i].disabled = false;
            }
        }
    ...
    function openRemoveModal(assignmentId, projectName, consultantName) {
        ...
        document.getElementById('remove_consultant_name').textContent = consultantName;
    ...
    document.getElementById('reassignModal').addEventListener('show.bs.modal', function(event) {
        ...
        if (button) {
            ...
            // Disable selecting the current consultant
            const newConsultantSelect = document.getElementById('new_consultant_id');
            for (let i = 0; i < newConsultantSelect.options.length; i++) {
                if (newConsultantSelect.options[i].value == consultantId) {
                    newConsultantSelect.options[i].disabled = true;
    ...
    document.getElementById('consultant_id').addEventListener('change', checkDuplicateAssignment);
